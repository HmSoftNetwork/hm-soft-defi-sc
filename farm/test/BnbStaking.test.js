...
contract('BnbStaking.......', async ([alice, bob, admin, dev, minter]) => {
  beforeEach(async () => {
    this.rewardToken = await CinemaDraftToken.new({ from: minter });
  ...
  it('deposit/withdraw', async () => {
    ...
    assert.equal((await this.rewardToken.balanceOf(alice)).toString(), '1200');
  ...
  it('emergencyRewardWithdraw', async () => {
    ...
    await this.bnbChef.emergencyRewardWithdraw(1000, { from: minter });
    assert.equal((await this.rewardToken.balanceOf(minter)).toString(), '99999999999901000');
  });
