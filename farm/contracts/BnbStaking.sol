...
contract BnbStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accCddxPerShare; // Accumulated CAKEs per share, times 1e12. See below.
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 accCddxPerShare = pool.accCddxPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 cakeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accCddxPerShare = accCddxPerShare.add(cakeReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accCddxPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accCddxPerShare = pool.accCddxPerShare.add(cakeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit() public payable {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCddxPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accCddxPerShare).div(1e12);
